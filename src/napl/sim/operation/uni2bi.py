...
class uni2bi(napl_base):
    """
    Convert unipolar spike streams to bipolar with scaled addition, please refer to
    ...
    def forward(self, input):
        ...
        if acc.shape == input.shape:
            ...
            acc = self.accumulator.data = acc.add(input).add_(1).clamp_(self.acc_min, self.acc_max)
