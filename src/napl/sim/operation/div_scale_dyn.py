...
class div_scale_dyn(napl_base):
    """
    Divide a spike stream by a fixed-point scale supplied at every timestep.

    Use this streaming accumulator when the divisor is a runtime input rather
    than a construction constant, as when a controller retunes the division
    while the stream runs. It supports unipolar and bipolar rate-coded inputs
    and takes one input stream, so it performs no reduction and returns an
    output of the same shape as its input.

    The target divisions for a call made with scale :math:`s` are

    .. math::

       p_y = \\frac{p_\\mathit{in}}{s}
       \\quad (\\text{unipolar}),\\qquad
       v_y = \\frac{v_\\mathit{in}}{s}
       \\quad (\\text{bipolar}).

    Every positive finite ``scale`` up to ``scale_max`` is supported: each call
    quantizes its ``scale`` to the nearest multiple of ``2 ** -fracwidth``, with
    a tie going to the even multiple, and a request rounding to zero raw units
    or rounding above ``scale_max`` raises. The threshold, the subtracted carry, and the
    bipolar centering offset ``(1 - scale) / 2`` all follow the quantized scale
    of the current call, so each output spike represents the scale in force at
    its fire time and a stream with a changing scale conserves mass against
    ``sum(quantized scale at each fire)``.
    ...
    A finite accumulator saturates the running sum, so the realized rate
    departs from the target once the accumulated input stays outside the
    representable range. For ``scale < 1`` the division amplifies: the input
    can add up to one unit per timestep while the accumulator drains by only
    ``scale``, so the accumulator rails and the output saturates at rate 1.
    ...
    .. code-block:: python
        ...
        divider = div_scale_dyn({'polarity': 'unipolar', 'scale_max': 4, 'intwidth': 8, 'fracwidth': 4})
        output = divider(torch.tensor([1, 0], dtype=torch.int8), 1.5)
    ...
    def __init__(
        ...
        """
        Configure the largest supported divisor and the fixed-point accumulator format.
        ...
        .. container:: api-parameter-list
            ...
            **Parameters:**
            ...
            - **config** – Configuration mapping.
              ...
              - **scale_max**: Sets the largest divisor a call may request. A positive finite number quantized to the nearest multiple of ``2 ** -fracwidth`` with ties going to the even multiple; that quantized multiple is the actual ceiling, so nearest-rounding may place it up to half of ``2 ** -fracwidth`` above the number given here. The default is ``2``.
        ...
        #: Smallest effective divisor, ``2 ** -fracwidth``.
        ...
        if type(requested) not in (int, float) or requested <= 0 or not math.isfinite(requested):
            ...
            logger.error(message)
            raise AssertionError(message)
        #: Largest requestable divisor in accumulator raw units.
        ...
        if self.scale_max_raw == 0:
            ...
            logger.error(message)
            raise AssertionError(message)
        #: Upper bound on a call's quantized divisor, the request quantized to the grid.
        ...
        if self.scale_max_raw > self.acc_max:
            message = (
                f'div_scale_dyn scale_max <{self.scale_max}> exceeds accumulator maximum '
        ...
        #: Divisor of the most recent call, the request quantized to the grid, or ``None`` before the first call.
        self.scale = None
    ...
    def _reset(self):
        """
        Clear the local accumulator and the reported divisor.

        The divisor returns to ``None`` because no call has selected one yet.
        """
    ...
    def forward(self, input, scale):
        ...
        Args:
            ...
            scale: Divisor applied on this call, a positive finite scalar
                quantized to the nearest multiple of ``2 ** -fracwidth`` with
                ties going to the even multiple. A request rounding to zero raw
                units or rounding above ``scale_max`` raises ``AssertionError``.

        Returns:
            A spike tensor shaped like ``input``. The call updates the
            accumulator, reports the quantized scale as :attr:`scale`, and
        ...
        **Example:**
        ...
        if type(scale) not in (int, float):
            message = f'div_scale_dyn scale must be an int or float scalar: got <{scale}>.'
            logger.error(message)
            raise AssertionError(message)
        # round() raises on a non-finite value, so the finiteness test runs before the quantization
        # below, where round() also resolves a scale sitting exactly between two grid points to the
        ...
        if scale_raw > self.scale_max_raw:
            message = (
                f'div_scale_dyn scale <{scale}> exceeds scale_max <{self.scale_max}>.'
            )
            logger.error(message)
            raise AssertionError(message)
        ...
        if self.polarity == 'bipolar':
            offset = (2**self.fracwidth - scale_raw)/2
