...
class linear_mix(napl_base):
    ...
    def __init__(
        ...
        if weight.dim() != 2:
            message = f'linear_mix weight must be 2D (out_features, in_features), got {tuple(weight.shape)}.'
        ...
        self.width = _check_acc_width('linear_mix', config.get('width', 12), self.entry,
        ...
        if config['generator'].lower() not in ['sobol', 'rc', 'rate']:
            logger.warning(
                f'linear_mix decorrelates operands via distinct sobol dimensions, but generator '
