...
from napl.sim.operation import add_scale, encode, mul_gaines


class butterfly_mix(napl_base):
    r"""
    Evaluate a radix-2 complex butterfly with Gaines multipliers on spike ports.

    Use this class to place a butterfly inside a fully streaming pipeline when
    the twiddle product should be an XNOR gate rather than a conditional-spike
    multiplier: it takes bipolar 0/1 spike tensors and returns bipolar 0/1 spike
    tensors, one timestep per call. The data ports carry no encoder and no
    decoder, so a caller encodes upstream and decodes downstream, and a chain of
    these classes never leaves the spike domain. This is the difference from
    :class:`fft_hub`, whose ports carry numeric values, and from
    :class:`butterfly_fp`, which is an exact binary-domain reference. The
    difference from :class:`butterfly_ugemm` is the multiplier: a Gaines gate
    needs both operands as streams, so the twiddle is encoded inside the class.
    ...
    construction and encoded into a spike stream inside the class rather than
    arriving on a port. A stream cannot represent a magnitude above one, so the
    returned streams encode the target divided by the adder scale reported in
    :attr:`compensation`,
    ...
    An XNOR multiplier is exact only for uncorrelated operands, so the twiddle
    encoder uses its own Sobol dimension, ``dim``, which must differ from the
    dimensions feeding the four input ports.
    ...
    .. code-block:: python
        ...
        from napl.sim.algorithm import butterfly_mix
        ...
        operation = butterfly_mix(twiddle_real, twiddle_imag, codec, adder)
        source = encode(codec)
        sink = decode(codec)
    ...
    def __init__(
        ...
        .. container:: api-parameter-list
            ...
            **Parameters:**
            ...
            - **mul_config** – Gaines multiplier and twiddle-encoder
              configuration.

              - **polarity**: Required stream encoding, which must be
                ``"bipolar"``.
              - **timestep**: Required positive twiddle-encoder stream length.
    ...
    def _reset(self):
        ...
        timestep, the twiddle encoder, the multiplier, and the adder before this
        hook returns ``None``.
        """
        pass
    ...
    def forward(self, x0r, x0i, x1r, x1i):
        ...
        The call advances the twiddle encoder, the multiplier, and the adder once
        and increments this module's timestep. The inputs are not modified.
        ...
        **Example:**
        ...
        if not (x0r.shape == x0i.shape == x1r.shape == x1i.shape):
            message = (
                f'butterfly_mix input shapes must match: got <{x0r.shape}>, '
        ...
        if x0r.ndim == 0 or x0r.shape[0] != self.lane:
            message = (
                f'butterfly_mix first input dimension must equal lane <{self.lane}>: '
