...
from napl.module.linear import (_linear_fxp_fn, _linear_hub_fn,
...
def _conv2d_binary(input, weight, bias, kernel_size, stride, padding, dilation, linear_fn):
    ...
    out = torch.nn.functional.fold(mm, out_hw, (1, 1))                                    # (b, out, H, W)
    if bias is not None:
...
class conv_tlut(napl_base):
    ...
    fxpfxp/fxpfp/fpfp via the (formati, formatw) pair. Single-shot, no tick; trains via STE.
...
class conv_fsu(napl_base):
    """
    Streaming (FSU) unary conv2d, computed bit by bit. Each timestep the weights are encoded
    ...
    def __init__(self, weight, bias=None, stride=1, padding=0, dilation=1,
        ...
        assert weight.dim() == 4, logger.error(f'conv_fsu weight must be 4D (out,in,kh,kw), got {tuple(weight.shape)}.')
        self.weight = weight
        ...
        assert 2 ** (width - 1) >= self.entry, logger.error(
            f'conv_fsu accumulator width <{width}> too small for fan-in <{self.entry}>: '
            f'2**(width-1) must be >= entry or partial sums saturate. Increase width.')
        if config['generator'].lower() not in ['sobol', 'rc', 'rate']:
            logger.warning(
                f'conv_fsu decorrelates operands via distinct sobol dimensions, but generator '
    ...
    def forward(self, input_spike):
        ...
        # materializing the (batch*L, out, K) elementwise product; bit-exact (small integers)
        psum = torch.matmul(inp, wf.t())                                     # (batch*L, out)
        if self.polarity == 'bipolar':
            psum = 2 * psum - inp.sum(-1, keepdim=True) - wf.sum(-1) + self.K
        if self.has_bias:
            psum = psum + self.b_encoder(self.bias).type(self.ntype)         # bias spike joins the sum
        acc = self.acc(psum, entry=self.entry, dim=None)                     # (batch*L, out)
        out = acc.reshape(input_spike.size(0), -1, acc.size(-1)).transpose(1, 2).type(self.ntype)  # (batch, out, L)
        return torch.nn.functional.fold(out, out_hw, (1, 1)).type(self.stype)  # (batch, out, H, W)
