...
class linear_fsu(napl_base):
    """
    Streaming (FSU) unary fully-connected layer: y = W x (+ b), computed bit by bit.
    ...
    def __init__(
        ...
        # lazy import: operation.mul imports module.encoder, so importing operation at
        ...
        assert weight.dim() == 2, logger.error(f'linear_fsu weight must be 2D (out_features, in_features), got {tuple(weight.shape)}.')
        self.weight = weight
        ...
        assert 2 ** (width - 1) >= self.entry, logger.error(
            f'linear_fsu accumulator width <{width}> too small for fan-in <{self.entry}>: '
        ...
        if config['generator'].lower() not in ['sobol', 'rc', 'rate']:
            logger.warning(
                f'linear_fsu decorrelates operands via distinct sobol dimensions, but generator '
...
class linear_fsu_pc(napl_base):
    """
    Streaming (FSU) unary linear *parallel counter*: the per-timestep binary inner-product
    count of the input spikes against freshly encoded weight spikes, before any accumulation
    into a bitstream. This is the `linear_fsu` partial sum without its scaled unary adder.
    ...
    def __init__(
        ...
        assert weight.dim() == 2, logger.error(f'linear_fsu_pc weight must be 2D (out_features, in_features), got {tuple(weight.shape)}.')
        self.weight = weight
        self.out_features, self.in_features = weight.shape
        self.has_bias = bias is not None
        self.entry = self.in_features + (1 if self.has_bias else 0)
        ...
        if config['generator'].lower() not in ['sobol', 'rc', 'rate']:
            logger.warning(
                f'linear_fsu_pc decorrelates operands via distinct sobol dimensions, but generator '
                f'<{config["generator"]}> does not decorrelate by dim (identical sequences across '
                f'operands). Use a sobol-family generator, or decorrelate the input and weight '
                f'streams by distinct seeds.')
        self.w_encoder = encoder({'polarity': self.polarity, 'timestep': config['timestep'],
...
class linear_fxp(napl_base):
    ...
    matmul, then shift the output back. Single-shot, no tick; trains via STE (exact linear
...
class linear_hub(napl_base):
    ...
    under the chosen RNG. Single-shot, no tick; trains via STE. Approximates nn.Linear
...
class linear_tlut(napl_base):
    ...
    from the (formati, formatw) pair: fxpfxp, fxpfp, fpfp. Single-shot, no tick; trains via
