...
// napl.sim.module.linear_mix (bipolar) over LANES output features.
...
// the popcount of the bipolar Gaines products. add_scale popcounts its ENTRY-bit
// input, so the lane is IN_FEATURES (encode -> mul_gaines_bipolar) cells plus the
// optional encoded bias spike feeding one add_scale_bipolar. Those are
// operation-layer circuits and are instantiated rather than rebuilt. Generated
    ...
    generate
        for (lane = 0; lane < LANES; lane = lane + 1) begin : g_lane
            for (feature = 0; feature < IN_FEATURES; feature = feature + 1) begin : g_mul
                ...
                // bipolar Gaines product with the input spike is an XNOR.
                wire w_spike;
                encode #(
                    .WIDTH    (SEQ_WIDTH),
                    .FRAC     (SEQ_WIDTH),
