...
// napl.sim.module.linear_pc (bipolar) over LANES output features.
// Each lane is one output feature and emits the parallel count of that feature's
// addends, not a spike: the layer's per-timestep count
// sum(x*w) + sum((1-x)*(1-w)) is, term by term,
// sum_f (1 - x_f - w_f + 2*x_f*w_f) = sum_f XNOR(x_f, w_f), so the count is the
// popcount of the bipolar Gaines products plus the optional bias spike.
// That is linear_bipolar with the add_any_bipolar accumulator removed and the
// popcount it reduces exposed, so the lane is IN_FEATURES mul_gaines_bipolar cells
// feeding an ENTRY-input popcount. mul_gaines_bipolar is an operation-layer
