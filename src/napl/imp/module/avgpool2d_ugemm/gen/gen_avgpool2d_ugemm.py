"""
Generate golden test vectors for the avgpool2d_ugemm module RTL straight from napl's
functional Python model (napl.sim.module.avgpool2d_ugemm) -- so the testbench checks the
Verilog against the *actual* simulator, not a hand-derived truth table.

avgpool2d_ugemm is stateful: each pooled output position keeps a residual accumulator
...
Output: ../vec/avgpool2d_ugemm.vec, one line per timestep:

    <rst> <in_bits> <out_bits>

`in_bits` is LANES*KERNEL_AREA binary digits, MSB first, so lane l occupies
i_input[l*KERNEL_AREA +: KERNEL_AREA] with the window flattened row-major.
...
../vec/avgpool2d_ugemm_params.vh, so the testbench elaborates the RTL at the model's
...
Run inside the `napl` conda env (so `import napl` resolves):
    python gen/gen_avgpool2d_ugemm.py
"""
import math
...
VEC = Path(__file__).resolve().parent.parent / "vec" / "avgpool2d_ugemm.vec"
PARAMS = Path(__file__).resolve().parent.parent / "vec" / "avgpool2d_ugemm_params.vh"

# Geometry, input shape, and codec mirror tests/module/test_avgpool2d_ugemm.py.
KERNEL_SIZE = 2
SHAPE = (4, 3, 8, 8)
TIMESTEPS = 256
...
def check_mapping():
    ...
    """
    node = {"class": "avgpool2d_ugemm", "config": {"kernel_size": KERNEL_SIZE, "lanes": LANES}}
    assert translate_node(node).parameters == {
        "KERNEL_AREA": KERNEL_AREA, "LANES": LANES}, \
        f"mapping.yaml avgpool2d_ugemm resolves {translate_node(node).parameters}"
...
def run_sequence(polarity, rows, dirty=0):
    ...
    enc = encode(codec)
    enc.reset()
    pool = avgpool2d_ugemm(KERNEL_SIZE, config={'polarity': polarity})
    pool.reset()
