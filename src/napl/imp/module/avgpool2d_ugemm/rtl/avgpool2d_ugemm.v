...
// napl.sim.module.avgpool2d_ugemm over LANES pooled output positions.
...
// subtract KERNEL_AREA on the emit. That is add_scale_unipolar with
...
module avgpool2d_ugemm #(
    ...
    input  wire                          i_clk,
    input  wire                          i_rst_n,
    input  wire [LANES*KERNEL_AREA-1:0]  i_input,         // lane l occupies bits [l*KERNEL_AREA +: KERNEL_AREA]
    ...
    generate
        for (lane = 0; lane < LANES; lane = lane + 1) begin : g_lane
            add_scale_unipolar #(
                ...
                .i_clk   (i_clk),
                .i_rst_n (i_rst_n),
                .i_input (i_input[lane*KERNEL_AREA +: KERNEL_AREA]),
