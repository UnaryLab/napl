...
// silently by %b.
// Co-sim: make test MODULE=mgu


module mgu_tb;
    localparam integer IN_FEATURES = `GEN_LANES + `GEN_IN_SIZE;
    localparam integer W_WIDTH     = `GEN_LANES * IN_FEATURES;
    localparam integer HXV_WIDTH   = `GEN_LANES * (`GEN_SEQ_WIDTH + 1);

    reg                     i_clk;
    reg                     i_rst_n;
    reg  [`GEN_IN_SIZE-1:0] i_input_spike;
    reg  [`GEN_LANES-1:0]   i_hx_spike;
    reg  [W_WIDTH-1:0]      i_weight_f;
    reg  [`GEN_LANES-1:0]   i_bias_f;
    reg  [W_WIDTH-1:0]      i_weight_n;
    reg  [`GEN_LANES-1:0]   i_bias_n;
    reg  [HXV_WIDTH-1:0]    i_hx_value;
    wire [`GEN_LANES-1:0]   o_out;
    wire [`GEN_LANES-1:0]   o_out_nb;
    ...
    reg  [W_WIDTH-1:0]      wf;
    reg  [`GEN_LANES-1:0]   bf;
    reg  [W_WIDTH-1:0]      wn;
    reg  [`GEN_LANES-1:0]   bn;
    ...
    initial begin
        i_clk         = 1'b0;
        i_rst_n       = 1'b1;
        i_input_spike = {`GEN_IN_SIZE{1'b0}};
        i_hx_spike    = {`GEN_LANES{1'b0}};
        i_weight_f    = {W_WIDTH{1'b0}};
        i_bias_f      = {`GEN_LANES{1'b0}};
        i_weight_n    = {W_WIDTH{1'b0}};
        i_bias_n      = {`GEN_LANES{1'b0}};
        i_hx_value    = {HXV_WIDTH{1'b0}};

        fd = $fopen("vec/mgu.vec", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open vec/mgu.vec (run `make vectors` first)");
            $finish;
        end
        ...
        while (code == 10) begin
            begin : g_row
                ...
                check_width(tok_bf, `GEN_LANES, "bf");
                check_width(tok_wn, W_WIDTH, "wn");
                check_width(tok_bn, `GEN_LANES, "bn");
                ...
                if (o_out !== exp_out) begin
                    $display("FAIL n=%0d bias : got %b exp %b", n, o_out, exp_out);
                    fails = fails + 1;
                end
                if (o_out_nb !== exp_out_nb) begin
                    $display("FAIL n=%0d nobias : got %b exp %b", n, o_out_nb, exp_out_nb);
                    fails = fails + 1;
                end
        ...
        if (fails == 0)
            $display("PASS mgu: %0d/%0d vectors (%0d lanes x %0d inputs, seq_width %0d, sr_width %0d)",
