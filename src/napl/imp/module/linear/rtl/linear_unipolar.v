...
// napl.sim.module.linear (unipolar) over LANES output features.
// Each lane is one output feature: the layer's per-timestep partial sum is
// sum_f x_f*w_f, the popcount of the unipolar Gaines products. add_any popcounts
// its ENTRY-bit input, so the lane is IN_FEATURES mul_gaines_unipolar cells plus
// the optional bias spike feeding one add_any_unipolar. Those are operation-layer
// circuits and are instantiated rather than rebuilt. Generated parameters mirror
// Python.
