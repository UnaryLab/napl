...
// napl.sim.module.conv_pc (bipolar) over LANES output pixel-channels.
...
// count sum(x*w) + sum((1-x)*(1-w)) over the im2col patch is, term by term,
// sum_k (1 - x_k - w_k + 2*x_k*w_k) = sum_k XNOR(x_k, w_k), so the count is the
// popcount of the bipolar Gaines products of the patch plus the optional bias spike.
// That is conv_bipolar with the add_any_bipolar accumulator removed and the
// popcount it reduces exposed, so the lane is K = IN_CHANNELS*KERNEL_H*KERNEL_W
// mul_gaines_bipolar cells feeding an ENTRY-input popcount. mul_gaines_bipolar is
