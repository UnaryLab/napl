...
// napl.sim.module.linear_ugemm (unipolar) over LANES output features.
...
//
// The Python model keeps one sequence index per input feature, shared by the
// whole weight column. Each lane here holds its own index register per input
// feature; every copy is advanced by the same input spike, so all copies carry
// the same value and the outputs are bit-exact with the shared-index model.


module linear_ugemm_unipolar #(
