...
// napl.sim.module.conv_mix (bipolar) over LANES output pixel-channels.
...
// KERNEL_W (encode -> mul_gaines_bipolar) cells plus the optional encoded bias spike
// feeding one add_scale_bipolar. Those are operation-layer circuits and are
...
module conv_mix_bipolar #(
    ...
    // The pad probability is the constant 0.5, whose code is LEN/2 = 2**(SEQ_WIDTH-1).
    localparam [OPW-1:0] PAD_CODE = (1 << (SEQ_WIDTH - 1));
    ...
    // One decorrelated rate-0.5 pad encoder shared by every padded tap: a padded
    // tap in the model reads one pad spike per timestep drawn from this stream.
    wire pad_spike;
    generate
        if (PADDING != 0) begin : g_pad_enc
            encode #(
                .WIDTH    (SEQ_WIDTH),
                .FRAC     (SEQ_WIDTH),
                .ROM_FILE (PAD_ROM)
        ...
        for (ow = 0; ow < OUT_W; ow = ow + 1) begin : g_out_col
            ...
            for (kw = 0; kw < KERNEL_W; kw = kw + 1) begin : g_tap_col
                ...
                if (IH >= 0 && IH < IN_H && IW >= 0 && IW < IN_W) begin : g_input
                    ...
                    assign x_bit = pad_spike;
                end

                // The weight code is re-encoded to a spike every timestep, then the
                // bipolar Gaines product with the patch spike is an XNOR.
                wire w_spike;
                encode #(
                    .WIDTH    (SEQ_WIDTH),
                    .FRAC     (SEQ_WIDTH),
