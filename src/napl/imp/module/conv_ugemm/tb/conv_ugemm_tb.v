...
// stream, in each of the three geometries: a (padding 1, bias), b (padding 0, no
// bias), c (padding 1, stride 2, dilation 2, bias). Weight and bias are held
// fixed-point codes read from vec/conv_ugemm_operand.hex, and the pad stream is
// built inside the DUT, so neither is a vector column.
...
module conv_ugemm_tb;
    ...
    initial begin
        ...
        if (fails == 0)
            $display("PASS conv_ugemm: %0d/%0d vectors (%0d, %0d and %0d lanes x %0d taps, scale %0d, %0d and %0d)",
                     n, n, `GEN_LANES_A, `GEN_LANES_B, `GEN_LANES_C, K,
