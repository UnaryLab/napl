...
// napl.sim.module.conv_ugemm (bipolar) over LANES output pixel-channels.
...
// lane is K = IN_CHANNELS*KERNEL_H*KERNEL_W mul_ugemm_bipolar cells and one
// encode cell feeding one add_scale_bipolar. Those are operation-layer circuits
...
// kw*DILATION - PADDING.
// A tap outside the input reads pad_bit, the alternating 0/1 zero-pad stream the
// model builds from its own timestep counter, here the jkff toggling on J=K=1.
// The pad tap drives its mul_ugemm cell like any other tap: it selects the
// input-one or the input-zero comparator path and it advances that cell's
// sequence index, so it is never a don't-care.
...
// Active-low reset clears every sequence index, the bias and pad counters, and
// every accumulator, to match reset().
...
module conv_ugemm_bipolar #(
    ...
    // Bipolar zero padding is the alternating 0/1 stream the model derives from
    // its timestep counter, starting at 0. A JK flip-flop held at J = K = 1 is
    // that toggle, and its reset state is the model's opening 0.
    wire pad_bit;

    generate
        if (PADDING != 0) begin : g_pad_toggle
            jkff u_pad (
                .i_clk     (i_clk),
                .i_rst_n   (i_rst_n),
                .i_input_j (1'b1),
