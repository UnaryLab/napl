"""
Generate golden test vectors for the add_desync RTL module straight from napl's
functional Python model (napl.sim.operation.add_desync) -- so the testbench checks
the Verilog against the *actual* simulator, not a hand-derived truth table.

add_desync is stateful: it holds a desync child whose signed saved-one counter lies
in [-DEPTH, DEPTH], and ORs the two desynchronized streams. reset() sets cnt = 0, side = +1.
...
Output: ../vec/add_desync.vec, one line per cycle:
...
op config (mirroring test_add_desync.py's make_operation), used to build the
model, AND emitted into ../vec/add_desync_params.vh as `GEN_DEPTH so the
...
VEC = Path(__file__).resolve().parent.parent / "vec" / "add_desync.vec"
PARAMS = Path(__file__).resolve().parent.parent / "vec" / "add_desync_params.vh"

# Sizing mirrors test_add_desync.py's make_operation.
DEPTH = 1
...
def state(model):
    """Saved-one counter and saving side of the model's correlation child."""
    return (int(model.desync.cnt.item()), int(model.desync.side.item()))
...
def main():
    models = [add_desync(config={"polarity": p, "depth": DEPTH})
