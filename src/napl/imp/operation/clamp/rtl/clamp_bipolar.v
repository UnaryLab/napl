...
// Bipolar napl.sim.operation.clamp: a saturating counter that clamps the running
...
// +/-1 value contributions, t the 1-based timestep, and em the emitted-spike count,
// the model fires when target_count(t) - em >= 0.5 with
//   target_count = (clamp(acc/t, lo, hi) * 0.5 + 0.5) * t.
...
module clamp_bipolar #(
    parameter integer LO        = -128,  // config['lo'] * 2^FRACWIDTH (grid units)
    parameter integer HI        =  128,  // config['hi'] * 2^FRACWIDTH (grid units)
    ...
    localparam integer ONE_F = (1 << FRACWIDTH);      // 2^FRACWIDTH: one grid unit
    // Signed compare bus: |LHS|,|RHS| <= (2*TIMESTEP+1)*2^FRACWIDTH; +1 for sign,
    // +2 headroom so t_cur*(HI+ONE_F) and (acc+t)<<FRACWIDTH never overflow.
    localparam integer CMP_W = TS_W + FRACWIDTH + 3;

    reg  [TS_W-1:0]        ts;    // completed timesteps
    reg  signed [TS_W:0]   acc;   // running sum of +/-1 steps, in [-TIMESTEP, TIMESTEP]
    ...
    wire signed [CMP_W-1:0] one_f = ONE_F[CMP_W-1:0];
    ...
    wire signed [CMP_W-1:0] lhs =
        below ? (t_cur * (lo_c + one_f)) :
        above ? (t_cur * (hi_c + one_f)) :
                ((a_cur + t_cur) <<< FRACWIDTH);
