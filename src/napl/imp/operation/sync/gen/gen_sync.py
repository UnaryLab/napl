"""
Generate golden test vectors for the sync RTL module straight from napl's
functional Python model (napl.sim.operation.sync) -- so the testbench checks the
Verilog against the *actual* simulator, not a hand-derived truth table.

sync is stateful: a signed saved-bit counter in [-DEPTH, DEPTH] pairs the
unpaired bits of the two streams. reset() sets cnt = 0. Both models are built,
one per polarity, and driven with the same stream: forward() reads no polarity
branch, so the two output streams must agree and one RTL module covers both.

Output: ../vec/sync.vec, one line per cycle:
...
op config (mirroring test_sync.py's make_operation), used to build the model,
AND emitted into ../vec/sync_params.vh as `GEN_DEPTH so the testbench overrides
the RTL parameter with the same value. RTL and sim therefore inherit DEPTH from
one place; they cannot drift. The header also records pp_delay and the row count.

Run inside the `napl` conda env (so `import napl` resolves):
    python gen/gen_sync.py
"""
import sys
...
VEC = Path(__file__).resolve().parent.parent / "vec" / "sync.vec"
PARAMS = Path(__file__).resolve().parent.parent / "vec" / "sync_params.vh"

# Sizing mirrors test_sync.py's make_operation.
DEPTH = 1
...
def emit_segment(models, f, stream, seen):
    ...
    for a, b in stream:
        ...
        seen.add(int(models[0].cnt.item()))
...
def main():
    models = [sync(config={"polarity": p, "depth": DEPTH})
