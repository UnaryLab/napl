...
`include "sync/vec/sync_params.vh"
...
module sync_tb;
    ...
    initial begin
        ...
        fd = $fopen("vec/sync.vec", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open vec/sync.vec (run `make vectors` first)");
            $finish;
        end
        ...
        if (`GEN_PP_DELAY != 0) begin
            $display("FAIL sync: observed latency 0, expected pp_delay %0d", `GEN_PP_DELAY);
            fails = fails + 1;
        end
        while (!$feof(fd)) begin
        ...
        if (n != `GEN_VECTORS) begin
            $display("FAIL sync: consumed %0d rows, generator emitted %0d", n, `GEN_VECTORS);
            fails = fails + 1;
        end

        if (fails == 0)
            $display("PASS sync: %0d/%0d vectors", n, n);
        else
            $display("FAIL sync: %0d mismatch(es) over %0d vectors", fails, n);
        $finish;
    end
endmodule
