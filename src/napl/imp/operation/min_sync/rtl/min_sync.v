...
// Minimum of two rate-coded streams, equivalent to napl.sim.operation.min_sync;
// pp_delay=0. The embedded synchronizer datapath (an inlined copy of the
// standalone sync operation, covered by a PAIRS row in test_equiv.py that walks
// it against the standalone) raises the
// correlation of the two streams, and an AND gate then returns the smaller
// rate. The circuit is identical for both polarities, so there is no polarity
// variant.
// Active-low reset clears the saved-bit counter.
...
module min_sync #(
    ...
    output wire o_output    // spike stream carrying the smaller of the two values
);
    ...
    wire sync_0 = agree | (is_1_0 & (saved_1 | full_0)) | (is_0_1 & saved_0);
    wire sync_1 = agree | (is_1_0 & saved_1) | (is_0_1 & (saved_0 | full_1));

    assign o_output = sync_0 & sync_1;
