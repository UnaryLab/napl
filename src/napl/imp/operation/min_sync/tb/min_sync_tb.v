...
`include "min_sync/vec/min_sync_params.vh"
...
module min_sync_tb;
    ...
    initial begin
        ...
        fd = $fopen("vec/min_sync.vec", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open vec/min_sync.vec (run `make vectors` first)");
            $finish;
        end
        ...
        if (`GEN_PP_DELAY != 0) begin
            $display("FAIL min_sync: observed latency 0, expected pp_delay %0d", `GEN_PP_DELAY);
            fails = fails + 1;
        end
        while (!$feof(fd)) begin
        ...
        if (n != `GEN_VECTORS) begin
            $display("FAIL min_sync: consumed %0d rows, generator emitted %0d", n, `GEN_VECTORS);
            fails = fails + 1;
        end

        if (fails == 0)
            $display("PASS min_sync: %0d/%0d vectors", n, n);
        else
            $display("FAIL min_sync: %0d mismatch(es) over %0d vectors", fails, n);
        $finish;
    end
endmodule
