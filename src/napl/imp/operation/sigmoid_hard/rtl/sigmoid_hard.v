...
// Streaming sigmoid_hard equivalent to add_scale(scale=2) on input+1.
...
// The model's `width` config sets the internal add_scale intwidth, which sets
// only the accumulator clamp bounds [-2**(width-1), 2**(width-1)-1]. It has no
// RTL parameter because those bounds are unreachable at every width the model
// accepts: add_scale rejects width below 3 (scale 2 exceeds its accumulator
// maximum), and at width 3 the clamp is already [-4, 3], outside the [0, 1] the
// accumulator holds. So no width changes a single output bit.
...
// inside the model's [-4, 3] bounds at the narrowest legal intwidth 3 and inside
// every wider one: neither clamp arm can ever be taken.
...
module sigmoid_hard (
    ...
    output wire o_output  // output spike (0/1)
);
