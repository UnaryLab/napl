...
`include "div_scale/vec/div_scale_params.vh"
...
module div_scale_tb;
    ...
    initial begin
        fd = $fopen("vec/div_scale.vec", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open vec/div_scale.vec (run `make vectors` first)");
            $finish;
        end
        ...
        if (fails == 0)
            $display("PASS div_scale: %0d/%0d vectors (unipolar + bipolar)", n, n);
        else
            $display("FAIL div_scale: %0d mismatch(es) over %0d vectors", fails, n);
        $finish;
    end
endmodule
