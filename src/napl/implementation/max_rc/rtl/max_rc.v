...
//
// Matches napl.operation.max_rc forward(): o_max uses the OLD dff, o_arg is the
...
module max_rc (
    ...
    // ---- sync_skewed(input_1=i_in_0, input_2=i_in_1) -> sync_0, i_in_1 ----
    wire input_01_10 = i_in_0 ^ i_in_1;              // (i_in_0 + i_in_1) == 1
    wire cnt_not_min = (cnt != 2'd0);
    wire cnt_not_max = (cnt != CNT_MAX);
    // select = cnt_not_min - (cnt_not_min + cnt_not_max) * i_in_0, in {-1,0,1}
    ...
    wire signed [2:0] select =
        ...
        - (i_in_0 ? $signed({1'b0, not_sum}) : 3'sd0);
    // sync_0 = i_in_0 + input_01_10 * select, provably in {0,1}; low bit is the
    // spike. Upper bits of the sum are always 0 here (intentionally unused).
    /* verilator lint_off UNUSEDSIGNAL */
    wire signed [2:0] sync_0_sum = $signed({2'b00, i_in_0})
        + (input_01_10 ? select : 3'sd0);
    /* verilator lint_on UNUSEDSIGNAL */
    wire sync_0 = sync_0_sum[0];
    wire sync_1 = i_in_1;

    // cnt_next = clamp(cnt + input_01_10 * (2*i_in_0 - 1), 0, CNT_MAX)
    wire signed [3:0] cnt_step  = input_01_10 ? (i_in_0 ? 4'sd1 : -4'sd1) : 4'sd0;
    ...
    assign o_max = dff ? i_in_1 : i_in_0;   // OLD dff
