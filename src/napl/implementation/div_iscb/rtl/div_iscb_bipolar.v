...
//
// RTL counterpart of napl.operation.div_iscb with config polarity='bipolar'
// (src/napl/operation/div.py), i.e. bipolar_forward:
//
//   (sign_d, abs_d) = sign_abs(dividend)            width 3
//   (sign_s, abs_s) = sign_abs(divisor)             width 3
...
// reset() state of each (sign_abs.acc=4, bi2uni/uni2bi.acc=0, the unipolar
...
module div_iscb_bipolar (
    ...
    div_iscb_sign_abs u_sad (
        .i_clk(i_clk), .i_rst_n(i_rst_n),
        .i_in(i_dividend), .o_sign(sign_d), .o_abs(abs_d)
    );
    div_iscb_sign_abs u_sas (
        .i_clk(i_clk), .i_rst_n(i_rst_n),
