"""
Generate golden test vectors for the sign_abs RTL module straight from napl's
functional Python model (napl.operation.sign_abs) -- so the testbench checks the
Verilog against the *actual* simulator, not a hand-derived truth table.

sign_abs is stateful: a saturating up/down accumulator (reset to ACC_MED =
...
Output: ../vec/sign_abs.vec, one line per cycle:

    <in> <sign> <abs>     (each 0/1, space-separated)

The sizing param WIDTH is the single source of truth here: it is read from the
op config (mirroring test_sign_abs.py's sign_abs_config), used to build the
model, AND emitted into ../vec/sign_abs_params.vh as `GEN_WIDTH so the testbench
overrides the RTL parameter with the same value. RTL and sim therefore inherit
WIDTH from one place; they cannot drift.

sign_abs is stateful, so the stream includes a MID-STREAM reset() (and a reset
...
VEC = Path(__file__).resolve().parent.parent / "vec" / "sign_abs.vec"
PARAMS = Path(__file__).resolve().parent.parent / "vec" / "sign_abs_params.vh"

# test_sign_abs.py sign_abs_config: the sizing param the op is built with.
SIGN_ABS = {"width": 3}
# test_sign_abs.py codec_config: the encoder feeding sign_abs.
CODEC = {"polarity": "bipolar", "timestep": 256, "generator": "sobol", "dim": 1}


def main():
    model = sign_abs(config=SIGN_ABS)
    model.reset()
    ...
    PARAMS.write_text(f"`define GEN_WIDTH {SIGN_ABS['width']}\n")
    ...
    with VEC.open("w") as f:
        for bit, reset_flag in items:
            ...
            rows += 1
    print(f"wrote {VEC} ({rows} vectors, WIDTH={SIGN_ABS['width']}) and {PARAMS}")
