...
// GEN_WIDTH is emitted by gen/gen_sign_abs.py from the op config (= the test's
// sign_abs_config), so the DUT parameter is inherited from the Python model.
// iverilog resolves this include relative to the compile cwd (implementation/).
`include "sign_abs/vec/sign_abs_params.vh"
//==============================================================================
// Self-checking testbench for sign_abs.
//
// Reads golden vectors produced by gen/gen_sign_abs.py (from the napl Python
...
module sign_abs_tb;
    ...
    initial begin
        ...
        fd = $fopen("vec/sign_abs.vec", "r");
        if (fd == 0) begin
            $display("ERROR: cannot open vec/sign_abs.vec (run `make vectors` first)");
            $finish;
        end
        ...
        if (fails == 0)
            $display("PASS sign_abs: %0d/%0d vectors", n, n);
        else
            $display("FAIL sign_abs: %0d mismatch(es) over %0d vectors", fails, n);
        $finish;
    end
endmodule
