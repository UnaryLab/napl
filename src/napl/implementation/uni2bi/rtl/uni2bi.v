...
//
// RTL counterpart of napl.operation.uni2bi (src/napl/operation/uni2bi.py).
...
//   addend       = i_in ? 2 : 1            // (input + 1)
//   acc_clamped  = clamp(acc + addend, ACC_MIN, ACC_MAX)
//   o_out        = (acc_clamped >= 2)      // combinational in i_in and acc
...
module uni2bi #(
    ...
    input  wire i_in,      // input spike (unipolar stream)
    ...
    // addend = (input + 1): 1 when i_in=0, 2 when i_in=1. The unsized signed
    // decimal constants are sign-extended into the [WIDTH:0] signed context.
    wire signed [WIDTH:0] addend  = i_in ? 2 : 1;
