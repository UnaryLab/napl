...
//
// RTL counterpart of napl.operation.bi2uni (src/napl/operation/bi2uni.py).
...
//   addend       = i_in ? 1 : -1           // (2*input - 1)
//   acc_clamped  = clamp(acc + addend, ACC_MIN, ACC_MAX)
//   o_out        = (acc_clamped >= 1)       // combinational in i_in and acc
...
module bi2uni #(
    ...
    input  wire i_in,      // input spike (bipolar stream)
    ...
    // addend = (2*input - 1): -1 when i_in=0, +1 when i_in=1.
    wire signed [DW-1:0] addend  = i_in ? ONE : -ONE;
