...
NAPL ("neuro-adaptive programming language") is UnaryLab's PyTorch framework for programmable spike processing. It supports per-timestep spike-stream simulation and single-shot binary-domain kernels, with verified Verilog counterparts for a subset of operations.
...
The Verilog-2001 hardware counterpart of `operation/` lives under `src/napl/implementation/`. Follow [RULE_RTL.md](RULE_RTL.md) for the mandatory RTL design and verification rules plus the layout and commands.
