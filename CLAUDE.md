...
- **RTL co-simulation:** from `src/napl/imp/`, `conda run -n napl make test OP=mul_and`. Requires Icarus Verilog (`iverilog`/`vvp`) on PATH. See "RTL implementation" below.
- **End-to-end applications (`zoo/`):** three applications use NAPL's public API, each with its own README: `mlp/` (MLP3 on MNIST in the streaming unary domain: `train_fp.py` then `eval_unary.py --device cpu|mps`), `ubrain/` (uBrain BCI CNN+RNN, FP and HUB forms; `build_hub_from_fp()` shares FP weights into the HUB model), `usystolic/` (uSystolic convnet_mnist; `eval_sweep.py` sweeps HUB over cycles and FXP over bitwidths). Results land in each application's `results/*.csv`.
