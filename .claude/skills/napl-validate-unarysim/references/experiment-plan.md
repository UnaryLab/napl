...
- **Streaming (FSU) kernels & metrics:** encode -> run N timesteps -> decode; progressive
  error vs the analytic reference within a stochastic-computing bound (~1/sqrt(N)).
  Matches the existing `test_mul_and.py` -> `report_error` pattern.
...
reference for the math). **Python-only**: the `implementation/` RTL tree is untouched.

## Gap analysis (UnarySim main -> napl)

Already present (no work): FSUMul->mul_and/mul_csg, FSUAdd->add_any, FSUDiv/CORDIV->div_*,
FSUReLU->relu_*, FSUSqrt->sqrt_*, FSUSignAbs->sign_abs, FSUHardsigmoid->sigmoid_hard,
...
- **Round primitive:** Round/RoundSTE -> operation/round.py (round_ste, round_fxp)
- **Linear:** FSULinear(+PC) -> linear_fsu; HUB/FXP/TLUT -> linear_hub/_fxp/_tlut
- **Conv:** FSUConv2d(+PC) -> conv_fsu; HUB/FXP/TLUT -> conv_hub/_fxp/_tlut
- **RNN:** FSUMGUCell -> mgu_fsu; HUBMGUCell/HardMGUCell/HardMGUCellFXP -> mgu_hub/_hard/_hardfxp
...
- Variant as lowercase suffix (linear_fsu, linear_hub, ...).
- Every kernel is a `napl_base` subclass. FSU = per-timestep forward() under
  `@napl_sim_timesteps` with tick(); HUB/FXP/TLUT/Hard = single-shot, trainable via
...
- Gotchas: no `>>`/`<<` on float tensors (use pow2_lshift/pow2_rshift shims);
  ...
  operation.mul <-> module.encoder import cycle); conv_fsu bipolar zero-pad uses a
...
1. **[DONE]** Walking skeleton: metric/correlation + metric/stability + operation/round +
   linear_fsu, one test each, end-to-end green. SC-customized adversarial review passed
...
3. **[DONE]** conv_fsu (streaming, im2col + decorrelated rate-0.5 bipolar pad stream) +
   ...
   0.025; conv_fsu rmse 0.005 (pad 0 and 1), STE gradients flow, reset reproduces. 44/44 green.
...
4. **[DONE]** module/rnn.py MGU cells: mgu_hard (binary float, == manual hard-MGU exactly),
   mgu_hardfxp (round_fxp everywhere, rmse 0.010 vs mgu_hard), mgu_fsu (streaming inner cell:
   linear_fsu scale=1 = linear+hardtanh, sigmoid_hard, mul_csg for fg*hx, mul_and for fg*ng,
   add_any scale=1 output), mgu_hub (hybrid: runs mgu_fsu over 2**width cycles, decodes with
   accuracy metric; rmse 0.032 vs mgu_hard). Caught & fixed a circular import (rnn.py must
   lazy-import napl.operation inside __init__, like linear_fsu) that pytest masked but the
