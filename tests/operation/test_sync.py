"""Test sync streaming; gradients are exempt because it has no parameters."""

import torch

from napl.sim.base import global_config
from napl.sim.metric import correlation
from napl.sim.operation import decode, encode, sync
from napl.utils._shared_test import devices, streaming_suite


# Fig. 3a of the DATE 2018 paper, walked from the initial state S1 with save
# depth 1. Each row is (input_0, input_1, output_0, output_1).
FSM_D1_SEQUENCE = [
    (1, 0, 0, 0),  # S1 -> S0, save the unpaired input_0 bit
    (0, 1, 1, 1),  # S0 -> S1, pair the saved input_0 bit
    (0, 1, 0, 0),  # S1 -> S2, save the unpaired input_1 bit
    (0, 1, 0, 1),  # S2 -> S2, no room left, pass through
    (1, 0, 1, 1),  # S2 -> S1, pair the saved input_1 bit
    (1, 1, 1, 1),  # S1 -> S1, inputs agree
    (0, 0, 0, 0),  # S1 -> S1, inputs agree
    (1, 0, 0, 0),  # S1 -> S0, save the unpaired input_0 bit
    (1, 0, 1, 0),  # S0 -> S0, no room left, pass through
]


def make_operation(polarity, _timestep, _device):
    return sync({'polarity': polarity, 'depth': 1})
...
CONFIG = {
    # sync is unipolar only: pairing ones and zeros has no bipolar meaning.
    'polarities': ['unipolar'],
    'tolerance_scale': 2.0,
...
def test_sync_config():
    ...
    for config, message in [
            ({'polarity': 'bipolar', 'depth': 1},
             'Invalid polarity: <bipolar>; sync supports unipolar only.'),
            ({'depth': 1},
        ...
        try:
            ...
            raise AssertionError(f'sync accepted the invalid config <{config}>.')


def test_sync_known_sequence():
    """Reproduce the Fig. 3a depth-1 state machine step by step on a rank-2 input."""
    for device in devices():
        operation = sync({'polarity': 'unipolar', 'depth': 1}).to(device)
...
def test_sync_correlation():
    ...
    for device in devices():
        ...
        previous = None
        for depth in [1, 2, 4]:
            operation = sync({'polarity': 'unipolar', 'depth': depth}).to(device)
            ...
            assert output_scc > 0.9, output_scc
            assert output_scc > input_scc + 0.5, (input_scc, output_scc)
            # Only bits still saved at the end of the run are lost, at most depth each.
...
def test_sync():
    """Verify sync preserves the first unipolar stream value across the full range."""
    streaming_suite(CONFIG)


if __name__ == '__main__':
    test_sync_config()
    test_sync_known_sequence()
