...
def test_sync_known_sequence():
    """Reproduce the Fig. 3a depth-1 state machine step by step on a rank-2 input under both polarities."""
    for device in devices():
        # Polarity only reinterprets the rate, so both polarities emit these bits.
        for polarity in ('unipolar', 'bipolar'):
            operation = sync({'polarity': polarity, 'depth': 1}).to(device)
...
def test_sync():
    """Verify sync preserves the first stream value across the full unipolar and bipolar ranges."""
