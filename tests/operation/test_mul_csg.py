...
class napl_mul_csg(napl_base):
    ...
    def forward(self, input0, input1, timesteps=256):
        ...
        o_spike = self.mul(i_spike, input1)
...
def test_mul_csg():
    ...
    input0 = gen_rand_tensor(codec_config['mode'], shape=(1000,), bitwidth=math.log2(codec_config['timestep'])).type(global_config.ntype).to(device)
    input1 = gen_rand_tensor(codec_config['mode'], shape=(1000,), bitwidth=math.log2(codec_config['timestep'])).type(global_config.ntype).to(device)

    # generate the napl_mul_csg instance
    mul_inst = napl_mul_csg(codec_config, mul_config, acc_config).to(device)
    mul_inst(input0, input1, timesteps=codec_config['timestep'])
