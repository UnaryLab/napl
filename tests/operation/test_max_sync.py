...
def analytic_reference(values, _polarity):
    ...
    # maximum of the rates is the maximum of the values.
...
def test_max_sync_known_sequence():
    """Reproduce the Fig. 5a depth-1 composition step by step under both polarities, then reset and replay it."""
    for device in devices():
        # Polarity only reinterprets the rate, so both polarities emit these bits.
        for polarity in ('unipolar', 'bipolar'):
            operation = max_sync({'polarity': polarity, 'depth': 1}).to(device)
...
def test_max_sync_accuracy():
    ...
    for device in devices():
        ...
        for depth in [1, 2, 4]:
            ...
            print(f'[{device}][depth={depth}] max_sync rmse={error:.6f}')
...
def test_max_sync():
    """Verify max_sync tracks the analytic maximum across the full unipolar and bipolar ranges."""
