"""Test max_sync streaming; gradients are exempt because it has no parameters."""

import torch

from napl.sim.base import global_config
from napl.sim.operation import decode, encode, max_sync, sync
from napl.utils._shared_test import devices, streaming_suite


# Fig. 5a of the DATE 2018 paper: a synchronizer feeding an OR gate. Each row is
# (input_0, input_1, sync_0, sync_1, output), with the synchronizer columns taken
# from the Fig. 3a depth-1 walk and the output column their OR.
SEQUENCE_D1 = [
    ...
    (0, 1, 0, 1, 1),  # S2 -> S2, no room left, pass through
    (1, 0, 1, 1, 1),  # S2 -> S1, pair the saved input_1 bit
    (1, 1, 1, 1, 1),  # S1 -> S1, inputs agree
    (0, 0, 0, 0, 0),  # S1 -> S1, inputs agree
    (1, 0, 0, 0, 0),  # S1 -> S0, save the unpaired input_0 bit
    (1, 0, 1, 0, 1),  # S0 -> S0, no room left, pass through
]

# Accuracy of the synchronized maximum against max(p_0, p_1); a bare OR gate or
# an AND gate after the synchronizer both land above 0.1 on the same inputs.
ERROR_BOUND = 0.02


def make_operation(polarity, _timestep, _device):
    return max_sync({'polarity': polarity, 'depth': 1})
...
def known_answer_case(_polarity):
    ...
    return values, torch.tensor([1.0, 1.0, 0.5]), 2.0 / 256


CONFIG = {
    # max_sync is unipolar only: the synchronizer pairs ones and zeros, which
...
def test_max_sync_config():
    ...
    for config, message in [
            ({'polarity': 'bipolar', 'depth': 1},
             'Invalid polarity: <bipolar>; max_sync supports unipolar only.'),
            ({'depth': 1},
        ...
        try:
            ...
            raise AssertionError(f'max_sync accepted the invalid config <{config}>.')


def test_max_sync_known_sequence():
    """Reproduce the Fig. 5a depth-1 composition step by step, then reset and replay it."""
    for device in devices():
        operation = max_sync({'polarity': 'unipolar', 'depth': 1}).to(device)
...
def test_max_sync_accuracy():
    """Match max(p_0, p_1) at depths 1, 2, and 4, where a wrong composition does not."""
    timesteps = 256
    torch.manual_seed(0)
    ...
    for device in devices():
        ...
        reference = torch.maximum(value_0, value_1)

        previous = None
        for depth in [1, 2, 4]:
            operation = max_sync({'polarity': 'unipolar', 'depth': depth}).to(device)
            error = _rmse(_run(operation, value_0, value_1, timesteps, device), reference)
            print(f'[{device}][depth={depth}] max_sync rmse={error:.6f}, bound={ERROR_BOUND}')
...
def test_max_sync():
    """Verify max_sync tracks the analytic maximum across the full unipolar range."""
    streaming_suite(CONFIG)


if __name__ == '__main__':
    test_max_sync_config()
    test_max_sync_known_sequence()
    test_max_sync_accuracy()
    test_max_sync()
    print('Test passed.')
