...
class napl_conv(napl_base):
    def __init__(self, codec_config, weight, bias, stride, padding, conv_config):
        ...
        self.conv = conv_mix(weight, bias, stride=stride, padding=padding, config=conv_config)
...
def _kernel_specific_checks():
    ...
    # The core-forwarding properties are lazy, so they are covered only by access.
    ...
    try:
        ...
        raise AssertionError('b_encoder must raise AttributeError without a bias')
...
def test_conv_mix():
    """Verify conv_mix for both polarities against analytic and known-answer streams."""
