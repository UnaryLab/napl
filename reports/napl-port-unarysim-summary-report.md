...
| operation.relu_hub | HUBReLU | yes (diff 0) | bit-exact; both Hardtanh(0,scale), CPU+MPS | 0.0015 ms | 0.0089 ms |
...
| operation.tanh_hub | HUBHardtanh | yes (diff 0) | bit-exact vs HUBHardtanh incl. corners | 10.0 ms/1000 | 12.2 ms/1000 |
...
- **TLUTLinearFPFPFunction** - backward-only STE helper behind TLUTLinear (FPFP); folded into `_linear_tlut_fpfp_fn`.
- **TLUTLinearFXPFPFunction** - backward-only STE helper behind TLUTLinear (FXPFP); folded into `_linear_tlut_fxpfp_fn`.
- **TLUTLinearFXPFXPFunction** - backward-only STE helper behind TLUTLinear (FXPFXP); folded into `_linear_tlut_fxpfxp_fn`.
...
- **BinGen** - binary source generator (formerly SourceGen); folded into napl encoder/gen_num_seq codec.
