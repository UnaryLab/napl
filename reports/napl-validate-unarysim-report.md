...
| 2026-08-02 | metric.stability_flux | Stability ratio (S1/S2) | yes (deterministic, CPU+MPS) | max\|diff\|=0 all regimes; inf/nan corner cases match elementwise | 13.9 ms | 38.9 ms | bipolar sobol 20x50 + unipolar sobol 1000 T=256; const-ones, stream-vs-itself, ones-vs-zeros(inf), T=1(nan); thr=0.05; N=1000 T=256 timing |
...
| 2026-07-29 | operation.mul_shiftreg | FSUMul(static=False) | yes (per-timestep and decoded max abs diff 0) | exact on identical input streams, CPU+MPS | 16.4 ms unipolar; 26.9 ms bipolar | 21.6 ms unipolar; 27.8 ms bipolar | unipolar+bipolar; seed-0 random shape 4096 and known-answer corners; T=256 width=4 |
...
| 2026-07-29 | operation.relu_shiftreg | FSUReLU(encode=RC, shiftreg=True) | yes (per-timestep and decoded max abs diff 0) | exact on identical input streams, CPU+MPS | 4.7 ms | 15.4 ms | bipolar RC; seed-0 range shape 1024 and known-answer corners; T=256 depth=8 |
| 2026-07-29 | operation.relu_tc | FSUReLU(encode=TC) | yes (per-timestep and decoded max abs diff 0) | exact on identical temporal streams, CPU+MPS | 3.2 ms | 15.9 ms | bipolar TC; seed-0 range shape 1024 and known-answer corners; T=256 width=8 |
...
| 2026-07-29 | operation.signabs_interleave | FSUAbs(interleave=True) | yes (sign, magnitude, and decoded max abs diff 0) | exact on identical input streams, CPU+MPS | 4.9 ms | 19.5 ms | bipolar; seed-0 random shape 1024 and known-answer corners; T=256 width=5 |
| 2026-07-29 | operation.signabs_shiftreg | FSUAbs(shiftreg=True) | yes (sign, magnitude, and decoded max abs diff 0) | exact on identical input streams, CPU+MPS | 4.5 ms | 17.2 ms | bipolar; seed-0 random shape 1024 and known-answer corners; T=256 depth=8 |
