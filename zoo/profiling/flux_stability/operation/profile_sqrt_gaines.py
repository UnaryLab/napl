"""Flux-stability profile for the sqrt_gaines streaming operation."""

from napl.sim.operation import sqrt_gaines

from profile_common import profile_op


def profile():
    """Return the flux-stability result dicts for sqrt_gaines, one per supported polarity."""
    # The radicand stays in [0, 1] so the square root is real and lands in [0, 1],
    # inside the legal range for both polarities.
    return [
        profile_op(
            sqrt_gaines,
            ctor={'polarity': 'unipolar', 'width': 5, 'generator': 'sobol', 'dim': 4},
            inputs=[
                {'range': (0.0, 1.0), 'polarity': 'unipolar', 'shape': (16384,), 'dim': 1},
            ],
            reference=lambda values, polarity: values[0].sqrt(),
            timesteps=256,
            seed=0,
        ),
        profile_op(
            sqrt_gaines,
            ctor={'polarity': 'bipolar', 'width': 5, 'generator': 'sobol', 'dim': 4},
            inputs=[
                {'range': (0.0, 1.0), 'polarity': 'bipolar', 'shape': (16384,), 'dim': 1},
            ],
            reference=lambda values, polarity: values[0].sqrt(),
            timesteps=256,
            seed=0,
        ),
    ]


if __name__ == '__main__':
    for r in profile():
        print(r)
