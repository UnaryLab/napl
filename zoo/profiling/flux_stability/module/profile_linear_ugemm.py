"""Flux-stability profile for the linear_ugemm streaming module."""
...
def profile():
    """Return the flux-stability result dicts for linear_ugemm, one per supported polarity."""
    ...
    for polarity in ('unipolar', 'bipolar'):
        ...
        # Module weight sequence uses dim 1; activation uses dim 2 to decorrelate.
        make_op = lambda w=weight, p=polarity: linear_ugemm(
            w, None,
            {'polarity': p, 'timestep': 256, 'generator': 'sobol', 'dim': 1, 'scale': None, 'width': 12},
        )
        results.append(profile_module(
            make_op,
            activation={'range': (lo, hi), 'polarity': polarity, 'shape': (32, in_features),
                        'dim': 2, 'generator': 'sobol', 'reduce_dim': -1},
